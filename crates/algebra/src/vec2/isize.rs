...
impl From<(T, T)> for ISizeVec2 {
    fn from((x, y): (T, T)) -> Self {
        Self::new(x, y)
    }
}

impl Into<(T, T)> for ISizeVec2 {
    fn into(self) -> (T, T) {
        (self.x, self.y)
    }
}

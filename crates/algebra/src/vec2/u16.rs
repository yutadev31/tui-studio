...
impl PartialOrd for U16Vec2 {
    fn partial_cmp(&self, other: &Self) -> Option<Ordering> {
        Some(self.cmp(other))
    }
}

impl Ord for U16Vec2 {
...
impl From<(T, T)> for U16Vec2 {
    fn from((x, y): (T, T)) -> Self {
        Self::new(x, y)
    }
}

impl Into<(T, T)> for U16Vec2 {
    fn into(self) -> (T, T) {
        (self.x, self.y)
    }
}
